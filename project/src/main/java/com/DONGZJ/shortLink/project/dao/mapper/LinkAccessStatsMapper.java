...
import com.DONGZJ.shortLink.project.dao.entity.LinkAccessStatsDO;
...
public interface LinkAccessStatsMapper extends BaseMapper<LinkAccessStatsDO> {
    /**
     * 记录监控记录,唯一索引是 短链接_分组标识_日期_小时
     */
    @Insert("insert INTO t_link_access_stats (full_short_url, gid, date, pv, uv, uip, hour, weekday, del_flag)"+
            "VALUES( #{linkAccessStats.fullShortUrl}, #{linkAccessStats.gid}, #{linkAccessStats.date}, #{linkAccessStats.pv}, #{linkAccessStats.uv}, #{linkAccessStats.uip}, #{linkAccessStats.hour}, #{linkAccessStats.weekday}, 0)"+
            "ON DUPLICATE KEY UPDATE pv = pv +  #{linkAccessStats.pv}, uv = uv + #{linkAccessStats.uv}, uip = uip + #{linkAccessStats.uip};"
    )
    void shortLinkStats(@Param("linkAccessStats") LinkAccessStatsDO linkAccessStatsDO);

    /**
     * 根据短链接获取指定日期内基础监控数据,获取指定日期内每一天的pv uv uip
     */
    @Select("SELECT " +
            "    date, " +
            "    SUM(pv) AS pv, " +
    ...
    /**
     * 根据短链接获取指定日期内小时基础监控数据,指定日期内每个小时的数据
     */
    @Select("SELECT " +
            "    hour, " +
            "    SUM(pv) AS pv " +
