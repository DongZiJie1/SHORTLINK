...
import com.DONGZJ.shortLink.project.mq.idempotent.MessageQueueIdempotentHandler;
import com.DONGZJ.shortLink.project.mq.producer.DelayShortLinkStatsProducer;
...
import static com.DONGZJ.shortLink.project.comment.Constant.RedisKeyConstant.LOCK_GID_UPDATE_KEY;
...
public class ShortLinkStatsSaveConsumer implements StreamListener<String, MapRecord<String, String, String>> {
    ...
    public void onMessage(MapRecord<String, String, String> message) {
        String stream = message.getStream();//获得流
        RecordId id = message.getId();//获得 消息的id
        //判断是否有key 如果有这个key 代表已经消费过（但是不代表消费过程结束，）
        if (!messageQueueIdempotentHandler.isMessageProcessed(id.toString())) {
            // 判断当前的这个消息流程是否执行完成，
            // （指一些极端情况，比如已经添加了 消费过的key，但是还没有消费结束 断电，也来不及抛异常去删除 代表消费过的key）
            //如果执行完成，直接返回
            //如果没有执行完成，抛异常 继续去消费
        ...
        try {
            ...
            //之所以要删除 是因为redis的内存比较珍贵，消费结束的消息就直接删除了
